...
| **細かな設定** | ビュワー・画像・ユーザーページ・操作の 4 区分で、画質・先読みの枚数・サイドバーの出し方・無限スクロールなどを好みに合わせて調整できる |
...
拡張機能の表示言語は、pixiv の表示言語に合わせて切り替わります。
(設定画面は、最後に開いた pixiv のページの表示言語に合わせます)
...
表は pixiv が表示言語として選べる 7 言語です。「リクエスト次第」の言語で pixiv を表示しているときは、英語で表示します。
対応してほしい言語があれば、下の「バグ報告・質問・リクエスト」からお知らせください。
...
ツールバーのアイコンから開きます。ビュワー・画像・ユーザーページ・操作の区分ごとに、次の項目を細かく調整できます。
