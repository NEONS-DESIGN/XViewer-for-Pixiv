...
| **アクション** | いいね、ブックマーク、フォロー、シェア (X / Facebook / Pawoo / リンクのコピー)。別タブで済ませたいいねは二重に数えず、ログインが切れていればその旨を知らせる。自分の作品には 3 つとも出さない (pixiv 本体と同じ) |
...
| `←` `→` | 同じ作品のページ送り (原寸表示中も同じ) |
| `↑` `↓` | 前後の作品へ移動 |
| `Esc` | モーダルを閉じる (原寸表示やシェアメニューが開いていればそちらを先に閉じる) |
...
| `npm test` | `node --test` でテストを走らせる (786 tests) |
