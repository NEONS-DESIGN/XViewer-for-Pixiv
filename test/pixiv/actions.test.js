...
test('likeIllust は既にいいね済みなら true を返す', async () => {
	const { impl } = fakeApiFetch({ is_liked: true });
	assert.equal(await likeIllust('1', 'T', { fetchImpl: impl }), true);
});

test('addBookmark は公開ブックマークを追加して ID を返す', async () => {
	const { impl, calls } = fakeApiFetch({ last_bookmark_id: '38764433361', stacc_status_id: '1' });
...
test('addBookmark は非公開なら restrict に 1 を入れる', async () => {
	const { impl, calls } = fakeApiFetch({ last_bookmark_id: '1' });
...
test('unfollowUser は rpc_group_setting.php へ送る (追加と別のエンドポイント)', async () => {
	const { impl, calls } = fakeFetch({ json: { user_id: '934903' } });
...
test('followUser は中身のある配列 (エラー文言) を失敗として投げる', async () => {
	const { impl } = fakeFetch({ json: ['エラーが発生しました'] });
...
test('followUser は配列でない応答を失敗として投げる', async () => {
	...
	const { impl } = fakeFetch({ json: { error: false, message: '', body: [] } });
	await assert.rejects(followUser('934903', 'TOKEN', { fetchImpl: impl }), (error) => {
...
test('unfollowUser は送った ID が返ってくれば成功として扱う', async () => {
	...
	const { impl } = fakeFetch({ json: { user_id: 934903 } });
	await assert.doesNotReject(unfollowUser('934903', 'TOKEN', { fetchImpl: impl }));
});

test('unfollowUser は user_id が返らなければ失敗として投げる', async () => {
	// キーが無い / null / 配列 (フォロー側の形) のどれも「反映されなかった」扱い
	for (const json of [{}, { user_id: null }, []]) {
		const { impl } = fakeFetch({ json });
		await assert.rejects(unfollowUser('934903', 'TOKEN', { fetchImpl: impl }), (error) => {
			assert.equal(error.kind, 'api', `${JSON.stringify(json)} で api にならない`);
			return true;
		});
	}
});

test('unfollowUser は送ったのと違う ID が返っても失敗として投げる', async () => {
	const { impl } = fakeFetch({ json: { user_id: '1' } });
	await assert.rejects(unfollowUser('934903', 'TOKEN', { fetchImpl: impl }), (error) => {
		assert.equal(error.kind, 'api');
		return true;
	});
...
test('addBookmark は応答に ID が無ければ失敗として投げる', async () => {
	...
	const { impl } = fakeApiFetch({ stacc_status_id: '1' });
...
test('addBookmark は数値の ID を文字列にして返す', async () => {
	const { impl } = fakeApiFetch({ last_bookmark_id: 38764433361 });
...
test('postComment は type=comment の urlencoded を送る', async () => {
	const { impl, calls } = fakeApiFetch(POSTED);
...
test('postStamp は type=stamp と stamp_id を送る', async () => {
	const { impl, calls } = fakeApiFetch({ ...POSTED, comment: '', stamp_id: '304' });
...
test('postStamp も返信なら parent_id を送る', async () => {
	const { impl, calls } = fakeApiFetch({ ...POSTED, stamp_id: '304' });
...
test('comment_id が無ければ投稿は失敗として扱う', async () => {
	...
	const { impl } = fakeApiFetch({ user_id: '1', user_name: 'x' });
...
test('deleteComment は API がエラーを返したら投げる', async () => {
	...
	const { impl } = fakeFetch({ json: { error: true, message: '削除できません', body: null } });
	await assert.rejects(
		() => deleteComment('1', '2', 'T', { fetchImpl: impl }),
