...
test('既定では /ajax/user/{id}?full=1 を取りに行く', async () => {
	...
	await fetchUserProfile('54734418', { getJsonImpl });
...
test('同じユーザーは一度しか取りに行かない', async () => {
	...
	const { getJsonImpl, urls } = fakeGetJson();
	await fetchUserProfile('1', { getJsonImpl });
	await fetchUserProfile('1', { getJsonImpl });
...
test('取得中に同じユーザーを求められても 1 本にまとめる', async () => {
	...
	const [first, second] = await Promise.all([
		fetchUserProfile('1', { getJsonImpl }),
		fetchUserProfile('1', { getJsonImpl }),
	]);
...
test('ユーザーが違えば別に取りに行く', async () => {
	const { getJsonImpl, urls } = fakeGetJson();
	await fetchUserProfile('1', { getJsonImpl });
	await fetchUserProfile('2', { getJsonImpl });
...
test('失敗は覚えない。次に呼ばれたらもう一度取りに行く', async () => {
	...
	await assert.rejects(() => fetchUserProfile('1', { getJsonImpl }));
	assert.deepEqual(await fetchUserProfile('1', { getJsonImpl }), { name: '作者' });
...
test('差し替えが同期的に投げても失敗として返す', async () => {
	...
	await fetchUserProfile('1', { getJsonImpl: ok });
	assert.equal(urls.length, 1);
});
...
test('上限を超えたら最古のユーザーから捨てる', async () => {
	...
	for (let i = 0; i < USER_PROFILE_CACHE_LIMIT; i += 1) {
		await fetchUserProfile(String(i), { getJsonImpl });
	}
	assert.equal(urls.length, USER_PROFILE_CACHE_LIMIT);
	// まだ上限内。最古 (0) は残っている
	await fetchUserProfile('0', { getJsonImpl });
	assert.equal(urls.length, USER_PROFILE_CACHE_LIMIT);
	// 1 人増えると最古 (0) が押し出され、次に求めると取り直す (そのとき次に古い 1 が押し出される)
	await fetchUserProfile('new', { getJsonImpl });
	await fetchUserProfile('0', { getJsonImpl });
...
test('patchUserProfile は覚えた内容に差分を重ねる', async () => {
	...
	assert.deepEqual(await fetchUserProfile('1', { getJsonImpl }), { name: '作者', isFollowed: true });
	assert.equal(urls.length, 1);
});
...
test('patchUserProfile は取得中でも完了後の内容に差分を重ねる', async () => {
	...
	const pending = fetchUserProfile('1', { getJsonImpl });
...
test('patchUserProfile は覚えていないユーザーには何もしない', async () => {
	...
	const { getJsonImpl, urls } = fakeGetJson();
	await fetchUserProfile('nobody', { getJsonImpl });
	assert.equal(urls.length, 1);
});
